...
- **The Void:** Hardcoded, unmodifiable, always-resident holding room (`system:void`). Seeded by `bin::init::init_world` with `VoidRoom` + `RoomKey("system:void")` + `ROOM_NO_TELEPORT_IN | ROOM_NO_TELEPORT_OUT | ROOM_SILENT`; recreation is guaranteed via `ensure_void_room` so it is never permanently absent. Serves as (a) the character-creation spawn fallback when no content spawn point is valid (`server::login::creation` keeps the Void key un-healed; `finalize_character` places the player there), (b) the recall/home fallback for orphaned positions (`.expect` removed in favor of `ensure_void_room`), and (c) an imm secure-holding area. Core exposes guards in `core::util`: `is_void_room`, `is_staff` (access ≥ Immortal), and `is_void_isolated` (non-staff occupant). Isolation rules are enforced at the command layer: movement (out/in/follow), builder topology (`@dig`/`@link`/`@unlink`/`@portal`/delete/`@set`), recall/teleport (`EffectTemplate::Teleport`), and the `cmd_use` skill gate all reject with _"You cannot do that right now"_; death in the Void does not recall (`core::combat::handle_death` keeps the victim in place). Communication is room-local only: `say`/`emote` are allowed, while tells/reply/whisper/channels/shout/group chat are gated bidirectionally for isolated non-staff via `bin::commands::common` helpers (`void_gags_sending` for outbound, `void_blocks_delivery` for inbound), with staff senders bypassing. System/self feedback (prompts, regen/tick notices, staff `gecho`) is never filtered.
- **Persistence & Content Loading:** Two-tier SQLite persistence in WAL mode with dirty flushing every 5s and automated hot backups. Content is loaded from TOML files into `TemplateRegistry` with hot-reloading via `notify`; files that fail to parse are reported (never silently skipped) via startup warnings and `oxide-server --validate-content` preflight checks. A content library with zero usable spawn points is a startup **warning**, not an error — the Void fallback keeps new characters loadable.
...
- **Log level** comes from the `RUST_LOG` environment variable when set, otherwise from `[logging] log_level` (`error`, `warn`, `info`, `debug`, `trace`; default `info`). The filter is applied uniformly to the stdout, rolling-file, and spade broadcast fmt layers so all log sinks see the same verbosity. `RUST_LOG` is the escape hatch for short-lived deep debugging; normal production operation should rely on the configured default so dependency connection noise stays suppressed.
