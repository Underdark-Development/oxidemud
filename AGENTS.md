...
This is a **pre-initialization** Rust project. The repo has exactly two files: `.git` (fresh) and `ARCHITECTURE.md`. No `Cargo.toml`, no source code, no dependencies, no CI, no tests. **Start any work session by reading `ARCHITECTURE.md`** — it is the sole design spec and must be treated as ground truth.
...
Six crates under a root `Cargo.toml` workspace:
...
| 5 | OLC commands, Rhai scripting, hot-reload content |
| 6 | WebSocket, MCCP/GMCP/MXP/MSSP, REST API, profiling |
...
- **Dependency DAG** — `core` depends on nothing else. `server`, `data`, `scripting` depend on `core` only. `bin` depends on all. No circular deps.
